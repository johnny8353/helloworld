...
var readyFunction =function(){
	...
	var chart = getColumnChart(store1,dataArray,groupArray);
	...
	var toggleBtn = {
		...
		toggleHandler: function(btn, pressed) {
			...
			chart = pressed ?getBarChart(store1,dataArray,groupArray):getColumnChart(store1,dataArray,groupArray);
...
function getBarChart(store1,dataArray,groupArray)
{
	var chart = Ext.create('Ext.chart.Chart', {
		...
		series: [getBarSeries(groupArray,dataArray)]
	});
	return chart;
}

function getBarSeries(groupArray,dataArray)
{
	...
	var series = {
			...
			tips: {
                ...
                renderer: function(storeItem, item) {
					//item.yField 获取当前点击的列标签
                    this.setTitle(storeItem.get(groupArray[0]) + ': ' + storeItem.get(item.yField)+ ' 次 ') ;
                }
            },
			style: {
				opacity: 0.93
...
function getColumnChart(store1,dataArray,groupArray)
{
	var chart = Ext.create('Ext.chart.Chart', {
		style: 'background:#fff',
		animate: true,
		...
		series: [getColumnSeries(groupArray,dataArray)]
	});
	return chart;
}

function getColumnSeries(groupArray,dataArray)
{
	// Series. 序列是负责数据仓库中中的数据点的视觉呈现
	var series = {
			type: 'column',//柱形图
